...
import { Trophy, RefreshCw, Sparkles, Skull } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#11111b]/85 backdrop-blur-lg">
      ...
      >
        ...
        <h1 className="text-3xl font-black text-[#cdd6f4]">
          {isPlayerWinner ? 'ŞAMPİYON OLDUNUZ! 🏆' : 'KURT KAZANDI! 🐺'}
        </h1>
        <p className="text-sm text-[#a6adc8] mt-2">
          {isPlayerWinner
            ? 'Tebrikler! 100 puan barajına ilk ulaşan taraf siz oldunuz!'
            : 'Kurt 100 puana sizden önce ulaştı. Bir dahaki sefere daha iyi bir strateji dene!'}
        </p>

        {/* Final Scoreboard */}
        <div className="my-6 grid grid-cols-2 gap-4 w-full">
          <div className="p-4 rounded-2xl bg-[#1e1e2e] border border-[#313244] text-center">
            <span className="text-xs uppercase text-[#a6adc8] font-bold">Oyuncu</span>
            <p className="text-3xl font-extrabold text-[#89b4fa] mt-1">{playerTotalScore}</p>
            <span className="text-[10px] text-[#a6e3a1]">Hedef Aşıldı</span>
          </div>

          <div className="p-4 rounded-2xl bg-[#1e1e2e] border border-[#313244] text-center">
            <span className="text-xs uppercase text-[#a6adc8] font-bold">Kurt</span>
            <p className="text-3xl font-extrabold text-[#f38ba8] mt-1">{wolfTotalScore}</p>
            <span className="text-[10px] text-[#6c7086]">Toplam Skor</span>
          </div>
        </div>
        ...
        <div className="w-full bg-[#1e1e2e]/60 rounded-xl p-3 border border-[#313244] mb-6 text-xs text-[#a6adc8] space-y-1.5 text-left">
          <div className="flex justify-between">
            <span>Oynanan Tur Sayısı:</span>
            <span className="font-bold text-[#cdd6f4]">{stats.roundsPlayed}</span>
          </div>
          <div className="flex justify-between">
            <span>Kazanılan Turlar:</span>
            <span className="font-bold text-[#a6e3a1]">{stats.playerRoundsWon} Tur</span>
          </div>
          <div className="flex justify-between">
            <span>Yapılan Eşleştirmeler:</span>
            <span className="font-bold text-[#89b4fa]">{stats.totalMatchesMade}</span>
          </div>
          <div className="flex justify-between">
            <span>Toplama & Bölme Hamleleri:</span>
            <span className="font-bold text-[#cba6f7]">{stats.totalSumMoves + stats.totalSplitMoves}</span>
          </div>
          <div className="flex justify-between">
            <span>En Yüksek Tur Puanı:</span>
            <span className="font-bold text-[#f9e2af]">{stats.highestRoundScore} Puan</span>
          </div>
        </div>
